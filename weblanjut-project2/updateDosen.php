...
if (isset($_POST['submit'])) {
    ...
    $sql = "SELECT * FROM dosen WHERE namadosen LIKE '%$cari%' 
        limit $halaman_awal, $batas";
            $halAktif = 1;

} else {
    // Jika form tidak disubmit, tampilkan semua data
    $sql = "select * from dosen limit $halaman_awal, $batas";
}
    // Jika form tidak disubmit, tampilkan semua data
        ...
        <tbody>
            <?php
                ...
                while($row=mysqli_fetch_assoc($data)){
                    ...
                    <td>
                    <a class="btn btn-outline-primary btn-sm" href="editDosen.php?kode=<?php echo $row['id']?>">Edit</a>
                    <a class="btn btn-outline-danger btn-sm" href="hpsDosen.php?kode=<?php echo $row["id"]?>" id="linkHps" onclick="return confirm('Anda ingin menghapus data ini?')">Hapus</a>
                    </td>
